...
<head>
<meta charset="UTF-8">
<title>Connexion Commerçant - BujaMarket</title>
...
.field input {
  ...
  background: var(--input-bg); font-size: 15px; color: var(--text); transition: all 0.2s ease; outline: none;
}
.field input:focus { border-color: var(--netlify-teal); background: #fff; box-shadow: 0 0 0 4px rgba(32, 198, 183, 0.1); }

button {
  width: 100%; margin-top: 10px; padding: 14px; border: none; border-radius: 6px;
  background: var(--netlify-teal); color: var(--netlify-navy); font-size: 16px; font-weight: 700; cursor: pointer; transition: 0.2s;
}
...
<div class="card">
  ...
  <!-- Affichage des erreurs de session s'il y en a -->
  ...
  <!-- Formulaire de login -->
  <form method="post" action="index.php?page=commercant&action=loginCommercant">
    <div class="field">
      <label>Email Professionnel</label>
      <input type="email" name="email_commercant" placeholder="votre@email.com" required>
    </div>
